...
def abrir_historico():
    ...
    # Cria um frame para a tabela e a barra de rolagem
    frame_tabela = tk.Frame(tabela_janela)
    ...
    # Cria um widget de árvore para exibir a tabela
    tabela = ttk.Treeview(frame_tabela, yscrollcommand=scrollbar.set)
    tabela.pack(side=tk.LEFT, fill=tk.BOTH)

    # Configura a barra de rolagem para rolar a tabela
    ...
    for dado in dados:
        ...
        data = datetime.strftime(dado[2], '%d/%m/%Y')
        tabela.insert("", tk.END, values=(dado[0], dado[1], data, dado[3], dado[4], dado[5], dado[6], dado[7], dado[8], dado[9], dado[10]))
...
def pesquisarAtivo():
    ...
    dados = cur.fetchall()
    tabela_janela = tk.Toplevel()
    tabela_janela.geometry('800x600')
    tabela_janela.title("Investimentos")

    # Cria um frame para conter a barra de rolagem e o canvas
    frame = tk.Frame(tabela_janela)
    ...
    # Configura a barra de rolagem para rolar com o canvas
    scrollbar.config(command=canvas.yview)

    # Cria um frame para conter os widgets dos dados
    inner_frame = tk.Frame(canvas)
    canvas.create_window((0, 0), window=inner_frame, anchor=tk.NW)

    tabela = tk.Label(inner_frame, text="Investimentos", font=("Helvetica", 16, "bold"))
    tabela.grid(row=0, column=0, columnspan=6, padx=10, pady=10)

    # Criando as colunas
    colunas = ["Cód. Transação", "Código", "Data", "Quantidade", "Valor Unitário", "Taxa de Corretagem", "Tipo Transação",
               "Valor Operação", "Imposto", "Valor Total", "Retorno"]
    for i in range(len(colunas)):
        coluna_label = tk.Label(inner_frame, text=colunas[i], font=("Helvetica", 12, "bold"))
    ...
    for i in range(len(dados)):
        for j in range(len(dados[i])):
            if j == 2:  # Quando for o índice da data ele pega o valor vindo do banco de dados e formata no formato dd/mm/aa
                ...
                dado_label = tk.Label(inner_frame, text=data, font=("Helvetica", 12))
            else:
                dado_label = tk.Label(inner_frame, text=str(dados[i][j]), font=("Helvetica", 12))
            dado_label.grid(row=i + 2, column=j, padx=10, pady=5)

    # Configura a barra de rolagem para acompanhar o tamanho do conteúdo do canvas
